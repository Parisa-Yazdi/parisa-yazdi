...
const MenuItem: React.FC<MenuItemProps> = ({ MenuItemName, subPages, href }) => {
  ...
  return (
    <div className="group relative m-0 block cursor-pointer p-0  antialiased hover:bg-white hover:text-black group-hover:border-[#1B3764]">
      {href ? (
        <Link
          href={href}
          className="m-3 flex h-full w-full items-center text-white group-hover:bg-white group-hover:text-black"
        >
          {MenuItemName}
        </Link>
      ) : (
