...
export default function NavBar() {
  ...
  return (
    <>
      ...
      <nav className="sticky top-0 z-50 hidden h-16 w-full border-b-2 border-white bg-black md:justify-evenly lg:inline-flex lg:justify-center lg:gap-6 xl:gap-16">
        {menuItems.map((item, i) => (
          <MenuItem
            key={i}
            MenuItemName={item.name}
            subPages={item.subPages}
            href={item.href} // Ensure href is passed correctly
          />
        ))}
      </nav>
    </>
