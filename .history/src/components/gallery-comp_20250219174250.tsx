...
export function GalleryComp({ images }: any) {
  console.log('Gallery images on render:', images);
        ...
        {images &&
          images.map((image: any, i: number) => (
            <div key={i} className="group relative cursor-pointer" onClick={() => handleClick(i)}>
              {image.attributes.art.data?.[0]?.attributes?.formats?.medium ? (
                <Image
                  alt={`Art Piece ${i + 1}`}
