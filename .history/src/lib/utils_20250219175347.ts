...
export async function fetchGallery() {
  ...
  const response = await request.json();
  const allHaveMedium = response.data.every((gallery: any) =>
    gallery.attributes.art.data.every((art: any) => art.attributes.formats?.medium)
  );

  console.log('🚀 ~ All contain medium?', allHaveMedium);
